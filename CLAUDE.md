...
4. **web_fetch**: URL取得（一時ファイルに保存、コンテキストウィンドウを圧迫しない）
...
`web_fetch` ツールは取得したコンテンツを `.tmp/web_fetch/` ディレクトリに一時ファイルとして保存します。これにより：

- 大量のコンテンツを取得してもコンテキストウィンドウを圧迫しない
- LLM は必要に応じて `file_read` でファイルを読み込める
- 24時間以上経過した一時ファイルはサーバー起動時に自動削除される
